...
## 2026-08 — Manus Audit & Production Hardening Release

### Lulu API Correctness (verified against the official Lulu OpenAPI spec)
- **Quotes**: cost calculation now sends `shipping_level` (was `shipping_option`, silently ignored by Lulu) and shipping cost is now treated as the whole-shipment total (was multiplied by quantity, overstating 200-copy quotes ~200x).
- **Shipping options**: mapping fixed to the real response fields (`level`, `cost_excl_tax`, `total_days_min/max` instead of five nonexistent field names).
- **Address validation**: orders with invalid/missing addresses now fail loudly; fake substitutions (invalid US state → 'CA', missing phone → '0000000000') removed.
- **Reorders**: now use the real `POST /print-jobs/` with each line item's immutable `printable_id`; the nonexistent `reprint/` endpoint was removed.
- **Status lifecycle**: eleven official statuses now mapped (previously missing UNPAID, PAYMENT_IN_PROGRESS, PRODUCTION_READY, PRODUCTION_DELAYED — the entire early life of every order).
- **Status history**: every transition is stored in `status_history` with source, message, tracking URLs, and raw payload; the Orders Timeline is now real data, not fabricated.
- **Idempotency**: one canonical `orderService.placeOrder` path; duplicate GHL workflow executions return the existing order instead of creating duplicate Lulu print jobs. `external_id` now carries the internal order id (was the contact id, unusable for dedupe).
- **Webhooks**: per-location HMAC verification with each sub-account's own `client_secret`; webhook health endpoint and one-click reactivation; background reconcile loop backfills drifted orders.

### CRM Sync
- Per-location GHL pipeline/stage mapping replaces the global `GHL_PIPELINE_ID` env vars; configured via the new CRM Pipelines page. Stage moves happen automatically on every Lulu status transition.

### Bug Fixes
- Menu link injection now receives `appBaseUrl` and `frontendBaseUrl` (was one arg → `undefined/book-setup?...`).
- `GET /print-jobs/export-csv` moved before `GET /:jobId` (was unreachable).
- `ghl.getContact` wrapper shape fixed; Stripe webhook guarded against missing modules; phantom-row writes removed.
- `initDatabase()` no-op replaced with an automatic migration runner (`server/db/applyMigrations.js`, see migration 004).
- All GHL HTTP calls carry a 15s timeout; boot no longer crashes on an unreachable Supabase (`SKIP_DB_MIGRATIONS=true`).

### New Env Vars
- `LITERARYAPP_API_SECRET` (server) / `VITE_API_SECRET` (client) — authorizes merchant-only ops.
