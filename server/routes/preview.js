...
router.post('/validate', upload.fields([
  ...
  const { podPackageId, pageCount, locationId } = req.body;
  ...
  if (!skuValidation.valid) {
    return res.status(400).json({
      ...
      podPackageId,
      hint: 'Please complete all book options (Size, Ink, Quality, Binding, Paper, Cover Finish) before validating.'
    });
  }
  ...
  try {
    ...
    if (parsedPageCount > 0) {
      try {
        expectedCover = await lulu.calculateCoverDimensions(podPackageId, parsedPageCount, 'in', locationId);
      } catch (dimErr) {
    ...
    const interiorVal = await lulu.validateInteriorFile(interiorResult.url, podPackageId, locationId);

    // 5. Submit cover file to Lulu if provided
    let coverVal = null;
    if (coverResult) {
      coverVal = await lulu.validateCoverFile(coverResult.url, podPackageId, parsedPageCount, locationId);
    }
...
router.get('/:previewId/status', async (req, res) => {
  ...
  try {
    ...
    if (entry.interiorValidationId && !DONE_STATUSES.includes(entry.interiorStatus)) {
      const interior = await lulu.getInteriorValidationStatus(entry.interiorValidationId, entry.locationId);
    ...
    if (entry.coverValidationId && !DONE_STATUSES.includes(entry.coverStatus)) {
      const cover = await lulu.getCoverValidationStatus(entry.coverValidationId, entry.locationId);
