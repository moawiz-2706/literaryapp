...
router.get('/callback', async (req, res) => {
  ...
  if (!code) {
    return res.status(400).send(`
      <html><body style="font-family:sans-serif;text-align:center;padding:60px;">
        ...
        <p>No authorization code was received during installation.</p>
        <p>Please try installing the app again from your account app menu.</p>
      </body></html>
    `);
  }
  ...
  try {
    ...
    if (userType === 'Company') {
      ...
      return res.send(`
        <html><body style="font-family:sans-serif;text-align:center;padding:60px;">
          <div style="max-width:420px;margin:0 auto;">
            ...
            <p style="color:#6b7280;">The Literary App has been connected to your agency account.</p>
      ...
      try {
        await ghl.injectCustomMenuLinks(locationId, process.env.APP_BASE_URL || '', process.env.FRONTEND_BASE_URL || '');
    ...
    return res.status(500).send(`
      <html><body style="font-family:sans-serif;text-align:center;padding:60px;">
        <h2 style="color:#dc2626;">Installation Failed</h2>
        <p>There was an error during installation.</p>
...
router.get('/status', async (req, res) => {
  ...
  try {
    ...
    // A backend failure must be reported as an error state — never crash the
    // process and never let the frontend misread it as "installed".
    console.error(`[AppStatus] getGhlToken failed for ${locationId}:`, err.message);
    res.status(503).json({ error: 'Unable to verify installation status.', installed: false, locationId });
  }
});
