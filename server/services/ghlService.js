...
async function getCustomValues(locationId) {
  const token = await getValidToken(locationId);
  const resp = await axios.get(`${GHL_BASE}/locations/${locationId}/customValues`, { headers: ghlHeaders(token) });
...
async function getContact(locationId, contactId) {
  const token = await getValidToken(locationId);
  try {
    const resp = await axios.get(`${GHL_BASE}/contacts/${contactId}`, { headers: ghlHeaders(token) });
    return resp.data;
  } catch (err) {
...
async function getOpportunity(locationId, opportunityId) {
  const token = await getValidToken(locationId);
  const resp = await axios.get(`${GHL_BASE}/opportunities/${opportunityId}/`, { headers: ghlHeaders(token) });
  return resp.data;
}
