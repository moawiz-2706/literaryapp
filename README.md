...
5. Author connects their Lulu.com account via the Lulu Integration settings page.
...
9. GHL "New Book Order" workflow fires the Custom Workflow Action.
10. Middleware receives the action webhook, calculates the print cost, and charges the sub-account wallet $10.00.
11. If the charge succeeds, middleware submits the print job to Lulu using the sub-account's credentials.
12. Middleware returns the Job IDs to GHL so the workflow can continue.
13. Lulu fires `PRINT_JOB_STATUS_CHANGED` webhooks to `/webhooks/lulu?locationId={id}`.
14. Middleware updates GHL Contact custom fields, moves the Opportunity pipeline stage, and adds tags.
