...
const css = `
  ...
  .bof-step-complete.bof-editable {
    cursor: pointer; transition: all 0.15s ease;
    justify-content: space-between; user-select: none;
  }
  .bof-step-complete.bof-editable:hover {
    background: #E0F2FE; border-color: #7DD3FC; color: #0369A1;
  }
  .bof-step-complete .bof-edit-hint {
    font-size: 11px; font-weight: 600; color: #6B7280; display: inline-flex;
    align-items: center; gap: 4px; flex-shrink: 0; margin-left: 8px;
  }
  .bof-step-complete.bof-editable:hover .bof-edit-hint { color: #0369A1; }
  .bof-summary-header {
    display: flex; align-items: center; justify-content: space-between;
    margin-bottom: 12px;
  }
  .bof-summary-title { font-size: 15px; font-weight: 700; color: #111827; }
  .bof-change-btn {
  ...
  .bof-change-btn:hover { background: #2563EB; color: #fff; }
...
function StepComplete({ label, value, onEdit, editHint }) {
  if (onEdit) {
    return (
      <div className="bof-step-complete bof-editable" onClick={onEdit} role="button" tabIndex={0}
        onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') onEdit(); }}
        aria-label={`Edit ${label}`}
        title={`Click to change ${label.toLowerCase()}`}>
        <span>✅</span>
        <span>{label}: <span className="bof-value">{value}</span></span>
        <span className="bof-edit-hint">✏️ {editHint || 'Edit'}</span>
      </div>
    );
  }
  return (
...
export default function BookOptionsForm({ fullOptions, onChange, initialComponents, compact }) {
  ...
  // ── Edit-mode jump: clicking a completed summary returns to that step,
  // clearing every downstream selection so the flow stays consistent.
  function jumpToStep(step) {
    if (step <= 1) handleTrimSelect('');
    if (step === 2) { setInk(''); setQuality(''); setBinding(''); setPaper(''); setCoverFinish('MXX'); setAvailableQualities([]); setAvailableBindings([]); setAvailablePapers([]); setAvailableCoverFinishes([]); }
    if (step === 3) { setQuality(''); setBinding(''); setPaper(''); setCoverFinish('MXX'); setAvailableBindings([]); setAvailablePapers([]); setAvailableCoverFinishes([]); }
    if (step === 4) { setBinding(''); setPaper(''); setCoverFinish('MXX'); setAvailablePapers([]); setAvailableCoverFinishes([]); }
    if (step === 5) { setPaper(''); setCoverFinish('MXX'); setAvailableCoverFinishes([]); }
    if (step === 6) { setCoverFinish('MXX'); setAvailableCoverFinishes([]); }
  }
  ...
  return (
    <div className={compact ? 'bof-compact' : ''}>
      ...
      {currentStep === 2 && availableInks.length > 0 && (
        <div style={{ animation: 'fadeIn 0.3s ease' }}>
          <StepComplete label="Trim" value={labels.trim?.[trim] || trim} onEdit={() => jumpToStep(1)} />
          <div className="bof-step-header">
      ...
      {currentStep === 3 && availableQualities.length > 0 && (
        <div style={{ animation: 'fadeIn 0.3s ease' }}>
          <StepComplete label="Trim" value={labels.trim?.[trim] || trim} onEdit={() => jumpToStep(1)} />
          <StepComplete label="Interior Color" value={labels.ink?.[ink] || ink} onEdit={() => jumpToStep(2)} />
          <div className="bof-step-header">
      ...
      {currentStep === 4 && availableBindings.length > 0 && (
        <div style={{ animation: 'fadeIn 0.3s ease' }}>
          <StepComplete label="Trim" value={labels.trim?.[trim] || trim} onEdit={() => jumpToStep(1)} />
          <StepComplete label="Interior Color" value={labels.ink?.[ink] || ink} onEdit={() => jumpToStep(2)} />
          <StepComplete label="Quality" value={labels.quality?.[quality] || quality} onEdit={() => jumpToStep(3)} />
          <div className="bof-step-header">
      ...
      {currentStep === 5 && availablePapers.length > 0 && (
        <div style={{ animation: 'fadeIn 0.3s ease' }}>
          ...
          <StepComplete label="Interior Color" value={labels.ink?.[ink] || ink} onEdit={() => jumpToStep(2)} />
          <StepComplete label="Quality" value={labels.quality?.[quality] || quality} onEdit={() => jumpToStep(3)} />
          <StepComplete label="Binding" value={labels.binding?.[binding] || binding} onEdit={() => jumpToStep(4)} />
          <div className="bof-step-header">
      ...
      {currentStep === 6 && availableCoverFinishes.length > 0 && (
        <div style={{ animation: 'fadeIn 0.3s ease' }}>
          ...
          <StepComplete label="Quality" value={labels.quality?.[quality] || quality} onEdit={() => jumpToStep(3)} />
          <StepComplete label="Binding" value={labels.binding?.[binding] || binding} onEdit={() => jumpToStep(4)} />
          <StepComplete label="Paper" value={labels.paper?.[paper] || paper} onEdit={() => jumpToStep(5)} />
          <div className="bof-step-header">
      ...
      {/* All steps complete — show editable summary */}
      {currentStep >= 7 && (
        <div style={{ animation: 'fadeIn 0.3s ease' }}>
          <div className="bof-summary-header">
            <span className="bof-summary-title">✅ All specifications selected</span>
            <span className="bof-change-btn" onClick={() => jumpToStep(1)} role="button" tabIndex={0}
              onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') jumpToStep(1); }}
              aria-label="Change specifications">✏️ Change all</span>
          </div>
          <StepComplete label="Trim" value={labels.trim?.[trim] || trim} onEdit={() => jumpToStep(1)} />
          <StepComplete label="Interior Color" value={labels.ink?.[ink] || ink} onEdit={() => jumpToStep(2)} />
          <StepComplete label="Quality" value={labels.quality?.[quality] || quality} onEdit={() => jumpToStep(3)} />
          <StepComplete label="Binding" value={labels.binding?.[binding] || binding} onEdit={() => jumpToStep(4)} />
          <StepComplete label="Paper" value={labels.paper?.[paper] || paper} onEdit={() => jumpToStep(5)} />
          <StepComplete label="Cover" value={coverFinish === 'MXX' ? 'Matte' : coverFinish === 'GXX' ? 'Gloss' : coverFinish} onEdit={() => jumpToStep(6)} />
          {compact && (
