...
const css = `
  ...
  .fp-incomplete-notice {
    padding: 8px 16px; background: #FFFBEB; border-top: 1px solid #FEF3C7;
...
export default function FilePreview({ interiorFile, coverFile, podPackageId, pageCount }) {
  ...
  async function handleValidate() {
    ...
    // Safety net: never send a malformed SKU (e.g. mid-selection
    // "0600X0900.BW....MXX") to the Lulu API — it returns 400.
    const skuCheck = isValidPodPackageId(podPackageId);
    if (!skuCheck.valid) {
      setError(`Book options are incomplete: ${skuCheck.reason}`);
      return;
    }
      ...
      <div className="fp-container">
        <div className="fp-header">
          ...
          <button
            ...
            disabled={validating || !interiorFile || !podPackageId || !isValidPodPackageId(podPackageId).valid}
            title={!isValidPodPackageId(podPackageId).valid ? 'Please complete all book options (Size, Ink, Quality, Binding, Paper, Cover Finish) first' : ''}
          >
        ...
        {podPackageId && !isValidPodPackageId(podPackageId).valid && (
          <div className="fp-incomplete-notice">
            <strong>Options incomplete:</strong> {isValidPodPackageId(podPackageId).reason}
          </div>
        )}
