...
export default function BookSetupPage() {
  ...
  async function handleApproveProduct(bookId) {
    if (!window.confirm('Are you sure you want to approve this product? If a GHL product has not been created yet, it will be created now. Once approved, the product will be active in your subaccount and ready for customers to order.')) return;
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    {showShippingForm === book.id && (
                      <>
                        <Button
                          size="sm"
                          disabled={sampleOrdering === book.id || !shippingForm.street1 || !shippingForm.city || !shippingForm.postcode || !shippingForm.phone_number || (shippingForm.country_code === 'US' && !shippingForm.state_code)}
                    ...
                    {book.status === 'Ready' && !book.product_approved && (
                      sampleStatusMap[book.id]?.sampleStatus === 'delivered' ? (
                        ...
                        <Button variant="ghost" size="sm" onClick={() => handleApproveProduct(book.id)} title="Skip sample and publish directly — product will be created in GHL if not already">
                          Skip & Publish
                        </Button>
                      ) : null
                    )}
