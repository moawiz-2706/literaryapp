...
import { isValidPodPackageId } from '../utils/podPackageId';
...
export default function BookSetupPage() {
  ...
  // Refresh error details for books stuck in 'Error' status.
  // The polling effect only refreshes books that are 'Validating', so after a
  // page reload an Error book loses its validation_details and only the generic
  // message is shown. Fetch each Error book's status once on mount.
  useEffect(() => {
    const errored = books.filter(
      b => b.status === 'Error' && (!b.validation_details || typeof b.validation_details === 'string' || (b.validation_details && !b.validation_details.errors))
    );
    if (!errored.length) return;
    errored.forEach(book => {
      api.get(`/books/${book.id}/validation-status`)
        .then(resp => {
          const { status, validationError, validationDetails } = resp.data || {};
          if (validationDetails || validationError) {
            setBooks(prev => prev.map(b =>
              b.id === book.id
                ? { ...b, status, validation_error: validationError, validation_details: validationDetails }
                : b
            ));
          }
        })
  ...
  function validateForm() {
    ...
    if (bookComponents?.podPackageId && !isValidPodPackageId(bookComponents.podPackageId).valid) {
      errors.bookOptions = 'Some book options are still unselected — please complete the full options wizard before uploading.';
    }
  ...
  const [shippingForm, setShippingForm] = useState({
    name: '', street1: '', street2: '', city: '', state_code: '', country_code: 'US',
    postcode: '', phone_number: '', email: ''
  });
  // Sample orders require a contact email — Lulu sends the shipping notification
  // to it and will not accept an order without one.
  ...
  async function handleOrderSample(bookId) {
    ...
    if (!email || !EMAIL_RE.test(email)) {
      ...
      return setError('A valid contact email is required on the shipping address — Lulu sends the shipping notification to it and will not accept an order without one.');
    }
    try {
      const resp = await api.post('/samples/order', {
        locationId,
        bookId,
        shippingAddress: { ...shippingForm, name: (shippingForm.name || '').trim(), email: (shippingForm.email || '').trim() },
        shippingLevel: 'MAIL'
      });
      setSampleStatusMap(prev => ({ ...prev, [bookId]: resp.data }));
      setBooks(prev => prev.map(b =>
        b.id === bookId ? { ...b, sample_print_job_id: resp.data.luluPrintJobId || resp.data.printJobId, sample_status: resp.data.sampleStatus, _sampleDisplayStatus: 'Sample Ordered' } : b
      ));
  ...
  async function handleApproveProduct(bookId) {
    if (!window.confirm('Are you sure you want to approve this product? If a Literary App product has not been created yet, it will be created now. Once approved, the product will be active in your account and ready for customers to order.')) return;
        ...
        <Card style={{ marginBottom: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
            {[
              ...
              { step: '4', title: 'Ships Direct', desc: 'We print and ship directly to your reader using Lulu’s actual shipping rates.' }
            ].map(item => (
          ...
          <div style={{ marginTop: 20, padding: '14px 16px', background: '#EFF6FF', borderRadius: 8, border: '1px solid #BFDBFE' }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#1E40AF', marginBottom: 6 }}>Actual Lulu Shipping Rates</div>
            <div style={{ fontSize: 13, color: '#1D4ED8' }}>
              Shipping costs are calculated in real-time by Lulu based on your reader's address, book weight, and selected shipping level. No flat rates — you pay exactly what Lulu charges.
            </div>
          </div>
        </Card>
          ...
          <Card style={{ marginBottom: '24px' }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ display: 'flex', gap: '12px', paddingTop: '8px' }}>
                <Button type="submit" disabled={submitting || (!!bookComponents?.podPackageId && !isValidPodPackageId(bookComponents.podPackageId).valid)}>
                  {submitting
                    ? <><Spinner size={16} color={colors.white} />&nbsp; Uploading...</>
                    : 'Upload & Validate Book'}
                </Button>
                {bookComponents?.podPackageId && !isValidPodPackageId(bookComponents.podPackageId).valid && (
                  <span style={{ fontSize: 12, color: '#B45309', alignSelf: 'center' }}>
                    Complete all book options first
                  </span>
                )}
                  ...
                  <div style={{ flex: 1, minWidth: '200px' }}>
                    ...
                    {book.status === 'Ready' && book.ghl_product_id && (
                      <div style={{ marginTop: '8px', fontSize: '12px', color: colors.gray500 }}>
                        Product created in the Literary App. Order a sample to review, then approve to activate.
                      </div>
                    )}
                    ...
                    {sampleStatusMap[book.id] && (
                      <div style={{ marginTop: '8px', fontSize: '12px', color: colors.gray700 }}>
                        {(sampleStatusMap[book.id].sampleStatus === 'shipped' || sampleStatusMap[book.id].sampleStatus === 'ordered') && (sampleStatusMap[book.id].trackingUrl || sampleStatusMap[book.id].luluTracking) && (
                          <div style={{ marginTop: '4px' }}>
                            <a href={sampleStatusMap[book.id].trackingUrl || sampleStatusMap[book.id].luluTracking} target="_blank" rel="noopener noreferrer"
                    ...
                    {showShippingForm === book.id && (
                      <div style={{
                        ...
                        <div style={{ fontSize: '13px', fontWeight: '600', marginBottom: '4px', color: colors.gray900 }}>
                          Shipping Address for Sample
                        </div>
                        <div style={{ fontSize: '11px', color: colors.gray500, marginBottom: '8px' }}>
                          All fields marked * are required by Lulu for shipping
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                          <input type="text" placeholder="Recipient Name * (Lulu requirement)" value={shippingForm.name}
                          ...
                          {shippingForm.country_code === 'US' ? (
                            <select value={shippingForm.state_code}
                              onChange={e => setShippingForm(f => ({ ...f, state_code: e.target.value }))}
                              style={{ padding: '8px 12px', border: `1px solid ${colors.gray200}`, borderRadius: '6px', fontSize: '13px', background: '#fff' }}>
                            ...
                            <input type="text" placeholder="State/Province" value={shippingForm.state_code}
                              onChange={e => setShippingForm(f => ({ ...f, state_code: e.target.value }))}
                              style={{ padding: '8px 12px', border: `1px solid ${colors.gray200}`, borderRadius: '6px', fontSize: '13px' }} />
                          )}
                          ...
                          <input type="text" placeholder="ZIP Code *" value={shippingForm.postcode}
                            ...
                            style={{ padding: '8px 12px', border: `1px solid ${colors.gray200}`, borderRadius: '6px', fontSize: '13px' }} />
                          <input type="text" placeholder="Country Code (e.g. US)" value={shippingForm.country_code}
                            onChange={e => setShippingForm(f => ({ ...f, country_code: e.target.value }))}
                            style={{ padding: '8px 12px', border: `1px solid ${colors.gray200}`, borderRadius: '6px', fontSize: '13px' }} />
                          <input type="email" placeholder="Contact Email * (shipping notification)" value={shippingForm.email}
                            onChange={e => setShippingForm(f => ({ ...f, email: e.target.value }))}
                            style={{ gridCol: '1 / -1', padding: '8px 12px', border: `1px solid ${colors.gray200}`, borderRadius: '6px', fontSize: '13px' }} />
                          <input type="text" placeholder="Phone Number * (e.g. 5551234567)" value={shippingForm.phone_number}
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    {showShippingForm === book.id && (
                      <>
                        <Button
                          size="sm"
                          disabled={sampleOrdering === book.id || !shippingForm.name || !shippingForm.street1 || !shippingForm.city || !shippingForm.postcode || !shippingForm.phone_number || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(shippingForm.email) || (shippingForm.country_code === 'US' && !shippingForm.state_code)}
                    ...
                    {book.status === 'Ready' && !book.product_approved && (
                      sampleStatusMap[book.id]?.sampleStatus === 'delivered' ? (
                        ...
                        <Button variant="ghost" size="sm" onClick={() => handleApproveProduct(book.id)} title="Skip sample and publish directly — product will be created in the Literary App if not already">
                          Skip & Publish
                        </Button>
                      ) : null
                    )}
