          ...
          {activeTab === 'customer' && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                ...
                <Card style={{ padding: 16 }}>
                  <div style={{ fontSize: 12, color: colors.gray500, marginBottom: 4 }}>Opportunity</div>
              ...
              {order.contactInfo && (
                <Card style={{ marginTop: 16, padding: 16 }}>
                  <div style={{ fontSize: 12, color: colors.gray500, marginBottom: 8, fontWeight: 600, textTransform: 'uppercase' }}>Contact Details</div>
          ...
          {activeTab === 'timeline' && (
            <div>
              {order.statusHistory && order.statusHistory.length > 0 ? (
                <div>
                  {(order.statusHistory || []).slice().reverse().map((step, i) => (
                    <TimelineStep
                      key={step.id || i}
                      label={formatStatus(step.from_status || 'Created') + (step.to_status ? ` → ${formatStatus(step.to_status)}` : '')}
                      detail={step.source || undefined}
                      timestamp={step.updated_at || step.created_at}
                      active
                    />
                  ))}
                  <TimelineStep label="Current Status" detail={order.status || '-'} timestamp={order.updatedAt || order.updated_at} active />
                </div>
              ) : (
                <EmptyState
                  title="No status history recorded"
                  description="The order has not transitioned through any tracked stages yet. Once it syncs with Lulu, each status change will appear here."
                  icon="📜"
                />
              )}
            </div>
...
function TimelineStep({ label, timestamp, detail, active }) {
  return (
    <div style={{ display: 'flex', gap: 12, padding: '12px 0', position: 'relative' }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        {detail && <div style={{ fontSize: 12, color: colors.gray400, marginTop: 1 }}>{detail}</div>}
        {timestamp && <div style={{ fontSize: 12, color: colors.gray500, marginTop: 1 }}>
